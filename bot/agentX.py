        ...
        if hits:
            ...
            for h in hits[:2]:
                ...
                try:
                    ...
                    # Convert nutrition estimate into a readable string.  The API may
                    # return a JSON string (e.g. "{\"Calories\":200,\"Protein\":10g,...}")
                    # or a Python dict.  Parse into "Calories: 200 kcal, Protein: 10 g" format.
                    est_txt = ""
                    if isinstance(est, str):
                        try:
                            parsed = json.loads(est)
                            if isinstance(parsed, dict):
                                est_txt = ", ".join([f"{k}: {v}" for k, v in parsed.items()])
                            else:
                                est_txt = str(parsed)
                        except Exception:
                            est_txt = est
                        ...
                        try:
                            if isinstance(est, dict):
                                est_txt = ", ".join([f"{k}: {v}" for k, v in est.items()])
                            else:
                                est_txt = str(est)
                        except Exception:
                            est_txt = str(est)
